// SPDX-License-Identifier: MIT

pragma solidity ^0.8.21;

import {FlipNFT} from "../src/FlipNFT.sol";
import {DeployFlipNFT} from "../script/DeployFlipNFT.s.sol";
import {Test, console} from "forge-std/Test.sol";

contract TestDeploy is Test {
    DeployFlipNFT deployFlip;
    string public happySvgUri;
    string public sadSvgUri;
    string public baseUri;

    function setUp() external {
        happySvgUri = "";
        sadSvgUri = "";
        baseUri = "data:image/svg+xml;base64,";
        deployFlip = new DeployFlipNFT();
    }

    function testSvgToImageURI() public {
        // Arrange
        string memory happySvg = vm.readFile("./img/happy.svg");
        string memory sadSvg = vm.readFile("./img/sad.svg");
        // Act
        string memory happyUri = deployFlip.svgToImageURI(happySvg);
        string memory sadUri = deployFlip.svgToImageURI(sadSvg);
        // Assert
        assertEq(
            keccak256(abi.encodePacked(happyUri)),
            keccak256(abi.encodePacked(baseUri, happySvgUri))
        );
        assertEq(
            keccak256(abi.encodePacked(sadUri)),
            keccak256(abi.encodePacked(baseUri, sadSvgUri))
        );
    }
}
